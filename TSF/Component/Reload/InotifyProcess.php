...
class InotifyProcess
{
    ...
    public function __construct($server, $urls)
    {
        echo "启动了autoReload\n";
        $this->server = $server;
        ...
        if (!extension_loaded('inotify')) {
            throw new \Exception("require <inotify> extension");
        } else {
            $this->useInotify();
        }
    }
    ...
    public function useInotify()
    {
        ...
        foreach ($this->urls as $url) {
            $dir_iterator = new \RecursiveDirectoryIterator($this->monitor_dir . DIRECTORY_SEPARATOR . $url);
        ...
        // 监控inotify句柄可读事件
        swoole_event_add($this->inotifyFd, function ($inotify_fd) use ($monitor_files){
            // 读取有哪些文件事件
